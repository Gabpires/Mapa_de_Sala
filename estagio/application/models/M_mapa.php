        ...
        try {
            ...
            if ($retornoConsultaReservaTotal['codigo'] == 11 || $retornoConsultaReservaTotal['codigo'] == 7) {
                ...
                if ($retornoConsultaSala['codigo'] == 1) {
                    ...
                    $retornoConsultaHorario = $horarioObj->getConsultaHorario($codHorario, '', '');

                    if ($retornoConsultaHorario['codigo'] == 10) {
                        $turmaObj = new M_turma();
                        $retornoConsultaTurma = $turmaObj->getConsultaTurmaCod($codTurma);
            ...
            if ($retornoHorario->num_rows() > 0) {
                ...
                if ($retornoMapa->num_rows() > 0) {
                    ...
                    if (trim($linha->estatus) == 'D') {
                        ...
                        $dados = array(
                            'codigo' => 8,
                            'msg' => 'A data de ' . $dataReserva . 'está ocupada para a sala'
                        );
                    }
                } else {
                    $dados = array(
    ...
    public function consultar($codigo, $dataReserva, $codSala, $codHorario, $codTurma, $codProfessor)
    {
        try {
            ...
            if ($retorno->num_rows() > 0) {
                ...
                $dados = array(
                    'codigo' => 11,
                    'msg' => 'Agendamento(s) não encontrado.'
                );
            }
        } catch (Exception $e) {
            $dados = array(
    ...
    public function alterar($codigo, $dataReserva, $codSala, $codHorario, $codTurma, $codProfessor)
    {
        try {
            ...
            if ($retornoConsultaCodigo['codigo'] == 1) {
                //inicio da query para atualização
                ...
                if ($codSala != "") {
                    ...
                    if ($retornoConsultaSala['codigo'] == 1 || $retornoConsultaSala['codigo'] == 10) {
                        $query .= "sala = $codSala, ";
                    } else {
                        $dados = array(
                            'codigo' => $retornoConsultaSala['codigo'],
                ...
                if ($codHorario != "") {
                    ...
                    if ($retornoConsultaHorario['codigo'] == 1 || $retornoConsultaHorario['codigo'] == 10) {
                ...
                if ($codTurma != "") {
                    ...
                    if ($retornoConsultaTurma['codigo'] == 1 || $retornoConsultaTurma['codigo'] == 10) {
                ...
                if ($codProfessor != "") {
                    ...
                    if ($retornoConsultaProfessor['codigo'] == 1 || $retornoConsultaProfessor['codigo'] == 10) {
                ...
                if ($this->db->affected_rows() > 0) {
                    $dados = array(
                        'codigo' => 1,
                        'msg' => 'Agendamento atualizado corretamente.'
                    );
                } else {
                    $dados = array(
                        'codigo' => 9,
                        'msg' => 'Houve algum problema na atualização na tabela de agendamento.'
                    );
                }
            } else {
                $dados = array(
